...
# Ensure unique names by appending suffixes to duplicates
...
df_run_selected_long <- df_run_selected %>%
  pivot_longer(
    cols = -c(Average.Rt.min., Metabolite.name),
    names_to = "Samples",
    values_to = "Area"
  )
...
ggplot(df_run_selected_long, aes(x = Samples, y = log2(Area))) +
  geom_boxplot() +
  geom_jitter(width = 0.15, alpha = 0.5) +  # Added alpha for better visualization of points
  theme(axis.text.x = element_text(angle = 90))+  
  labs(x = "Samples", y = "Log2(Area)", title = "Box Plot of Log-Transformed Area by Sample")
...
sample_mv_columns <- names(df_run_selected)[!(names(df_run_selected) %in% c("Average.Rt.min.", "Metabolite.name", "Blank", "Pool_1", "Pool_2"))]
...
blank_thresholds <- normalized_data %>%
  ...
  summarise(across(everything(), ~ quantile(.x, 0.25, na.rm = TRUE)))
...
df_normalized_data_long <- normalized_data %>%
  ...
  mutate(flag = case_when(
    Samples == "Blank" & Area > blank_thresholds$Blank ~ "Above Q1",
    TRUE ~ "Below Q1"))
...
ggplot(df_normalized_data_long, aes(x = Samples, y = log10(Area))) +
  ...
  scale_color_manual(values = c("Above Q1" = "red", "Below Q1" = "black")) +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(x = "Samples", y = "Log10(Area)", title = "Box Plot of Log10(Area) by Sample with Flagged Metabolites")


# Step 4. Filter for Metabolites Below Q1 in All Blanks

#---- Filter for Metabolites Below Q1 in Blank ----
# Calculate the Q1 threshold for the blank sample
blank_Q1_threshold <- quantile(normalized_data$Blank, 0.25, na.rm = TRUE)

# Identify metabolites below Q1 in the blank
below_Q1_metabolites <- normalized_data %>%
  filter(Blank <= blank_Q1_threshold) %>%
  pull(Metabolite.name)

# Filter the normalized data to keep only metabolites below Q1 in the blank
df_below_Q1 <- normalized_data %>%
  filter(Metabolite.name %in% below_Q1_metabolites)





# Step 5. Create New PCA with df_below_Q1

#---- Create New PCA with df_below_Q1 ----
# Prepare data for PCA by selecting sample columns
df_pca_data <- df_below_Q1 %>%
...
# Plot PCA results with group colors
pca_data <- data.frame(Sample = sample_names, Group = group_names, PC1 = pca_result$x[,1], PC2 = pca_result$x[,2])
ggplot(pca_data, aes(x = PC1, y = PC2, color = Group, label = Sample)) +
  geom_point(size = 3) +
  labs(title = "PCA Plot of Samples by Group (Below Q1 Metabolites)", x = "Principal Component 1", y = "Principal Component 2") +
  theme_minimal() +
  theme(legend.position = "right")
...
df_with_blank_avg <- df_below_Q1 %>%
  rowwise() %>%
  mutate(Average_Blank = mean(c_across(starts_with("Blank")), na.rm = TRUE))

# Step 2: Define sample columns, excluding `Pool_1`, `Pool_2`, and all `Blank` columns
sample_columns <- setdiff(names(df_below_Q1), c("Metabolite.name", "Blank", "Pool_1", "Pool_2"))
