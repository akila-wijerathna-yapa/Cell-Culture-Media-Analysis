...
df_combined <- read.csv("data/Metab_merged.csv")
...
summary(df_combined_scaled %>% select(-Metabolite))
...
combined_group_colors <- setNames(combined_solid_colors, unique(df_combined_scaled_pca_data$Group))
...
combined_pc1_var <- round(combined_explained_variance[1] * 100, 2)  # PC1 percentage
combined_pc2_var <- round(combined_explained_variance[2] * 100, 2)  # PC2 percentage
...
combined_pca_plot <- ggplot(df_combined_scaled_pca_data, aes(x = PC1, y = PC2, color = Group, label = Sample)) +
  ...
  labs(
    ...
    x = paste0("Principal Component 1 (", combined_pc1_var, "%)"),
    y = paste0("Principal Component 2 (", combined_pc2_var, "%)"),
    color = "Group"
  ) +
  theme_minimal() +
  theme(
...
combined_heatmap_data <- df_combined_scaled %>% select(-Metabolite) %>% as.matrix()
...
df_combined_v3_Met_Avg <- df_combined_v3 %>%
  select(-Metabolite) %>% # Drop the specified columns
